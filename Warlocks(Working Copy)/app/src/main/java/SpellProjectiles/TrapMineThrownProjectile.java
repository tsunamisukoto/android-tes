...
public class TrapMineThrownProjectile extends Projectile {
    ...
    public TrapMineThrownProjectile(Vector _from, Vector _to, GameObject shooter, int Rank) {
        super(R.drawable.spell_trapmines,_from, _to, shooter,Rank);
...
this.height= 0;
    ...
    public void Update() {
        if(this.health%10==1)
            this.heightvel-=1;
        if(this.health<=0)
            SimpleGLRenderer.addObject(new TrapMineProjectile(Vector.Zero(),this.bounds.Center.get(), this.owner,this.Rank));
        super.Update();
